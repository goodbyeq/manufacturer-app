...
public class XssInputFilter {
    /**
     ...
     public XssInputFilter()
     {
         this(false);
     }

     public XssInputFilter(boolean debug)
     {
         vDebug = debug;
     ...
     //want to remove list of harmful strings appear user input
     public static String removeDefinedTags( String s )
     {
         s = s.replaceAll("eval\\((.*)\\)", "");
         s = s.replaceAll("alert\\((.*)\\)", "");
         s = s.replaceAll("[\\\"\\\'][\\s]*((?i)javascript):(.*)[\\\"\\\']", "\"\"");
     ...
     public synchronized String filter( String input )
     {
         ...
         s = removeDefinedTags(s);
         debug( "removeDefinedTags: " + s );
